...
for k in train_pipeline.keys():
    ...
    # Default values
    target_column = 'TargetText'                # Which column holds the text to be processed
    target_column_id = 'TargetID'               # Name of ID column related to target column
    GroupColumn = 'None'                        # Set None if no groupcolumn
    GroupValue = 'None'                         # Set None if no group column
    lda_sample_size = 100000                    # Setting max sample size for each subgroup
    ...
    lda_eta = 'auto'                            # A-priori belief on word probability
    lda_minimum_probability = 0.005             # Topics with a probability lower than this threshold will be filtered out.
    ...
    # Fixed values
    predict_by_column = 'Text_Clean'            # The name of the column in the dataframe used for prediction     
    random_state=1                              # Setting the seed for any sampling
...
for k in train_pipeline.keys():
    ...
    # Tokenize text
    df['Text_Tokenized'] = df[target_column].apply(lambda x: nlp(x))
    print(df[0:1])
            
    ###############
    #### Cleaning text for stopwords
    ###############
    ...
    # all_stopwords |= {"2008","forlod",} 

    # Define functions for stopword removal
    def remove_stopwords(text_tokens):
        return [word for word in text_tokens if word.is_stop == False]

    # apply function to tokenized text
    df['Text_StopwordsExcluded'] = df['Text_Tokenized'].apply(lambda x: remove_stopwords(x))
